...
function populateCategorySelect() {
  ...
  document.querySelector('.admin-cat-filter').innerHTML =
    cats.map(c => `<button class="cat-btn ${c === 'all' ? 'active' : ''}" data-cat="${c}" onclick="filterAdminCat(this,'${c}')">${c === 'all' ? '全部' : c}</button>`).join('');
}
...
function renderAdminList() {
  ...
  const activeBtn = document.querySelector('.admin-cat-filter .cat-btn.active');
